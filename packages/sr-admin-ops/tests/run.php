...
use StockResource\AdminOps\Plugin;
...
$entry = file_get_contents(dirname(__DIR__) . '/sr-admin-ops.php');
assert_true($entry !== false, 'plugin entry file is readable');
assert_true(str_contains($entry, 'Plugin Name: Stock Resource Admin Ops'), 'plugin header declares the admin ops plugin name');
...
assert_same('sr-admin-ops', Plugin::slug(), 'plugin exposes stable slug');
...
echo "sr-admin-ops skeleton tests: ok\n";
