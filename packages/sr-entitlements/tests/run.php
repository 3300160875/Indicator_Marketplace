...
$entry = file_get_contents(dirname(__DIR__) . '/sr-entitlements.php');
assert_true($entry !== false, 'plugin entry file is readable');
assert_true(str_contains($entry, 'Plugin Name: Stock Resource Entitlements'), 'plugin header declares the entitlements plugin name');
...
assert_same('sr-entitlements', Plugin::slug(), 'plugin exposes stable slug');
...
echo "sr-entitlements skeleton tests: ok\n";
