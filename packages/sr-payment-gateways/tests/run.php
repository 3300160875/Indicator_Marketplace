...
use StockResource\PaymentGateways\Plugin;
...
$entry = file_get_contents(dirname(__DIR__) . '/sr-payment-gateways.php');
assert_true($entry !== false, 'plugin entry file is readable');
assert_true(str_contains($entry, 'Plugin Name: Stock Resource Payment Gateways'), 'plugin header declares the payment gateways plugin name');
...
assert_same('sr-payment-gateways', Plugin::slug(), 'plugin exposes stable slug');
...
echo "sr-payment-gateways skeleton tests: ok\n";
