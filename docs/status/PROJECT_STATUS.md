...
- Next safe task: SR-035 用户订单列表与下载入口
- Gate 0: complete
...
- SR-034 已进入 REVIEW：订单项业务快照支持层已完成，商品类型/规则版本/价格/资源或套餐元数据冻结，已有快照按订单项幂等复用，非本人订单、退款单和缺失用户映射会安全失败。
...
1. 完成 SR-034 PR / CI / independent QA / merge / VERIFIED 收口。
2. SR-035 用户订单列表与下载入口。
3. SR-036 生成订单状态机与边界文案。
