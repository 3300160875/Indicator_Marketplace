...
- SR-035 已进入 REVIEW：用户订单列表与详情适配支持层已完成，只返回当前用户订单，状态映射为用户可读文案，缓存 key 包含用户与规则版本，空态/过期/撤权/配额重置有可复核投影，内部审核备注不会暴露。
...
1. 完成 SR-035 PR / CI / independent QA / merge / VERIFIED 收口。
