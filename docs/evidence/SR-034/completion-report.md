...
- Contract changes: `OrderSnapshotService` consumes `EddOrderAdapter` snapshots only; `OrderItemBusinessSnapshot` freezes product type, rules version, access mode, price, unit/discount/total amounts, resource/version or plan metadata, currency, refund status, terms versions and idempotency key.
- Migrations: none.
